...
def load_nifty_volume_as_array(filename, transpose=True, return_spacing=False, respacing=False, target_spacing=1,
    ...
    if respacing:
        ...
        if mode == 'image':
            ...
            data = np.int8(data)
            data = np.int8(resize_Multi_label_to_given_shape(data, zoom_factor=zoomfactor, order=order,
                                                             class_number=np.max(data) + 1))
...
def load_volume_as_array(filename, transpose=True, return_spacing=False, return_origin=False, respacing=False,
                         target_spacing=1,
                         mode='image', order=3):
    """
    load nifty image into numpy array, and transpose it based on the [z,y,x] axis order
